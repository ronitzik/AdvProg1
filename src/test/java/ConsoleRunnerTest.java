...
public class ConsoleRunnerTest {
    ...
    public void testMultiPlayerGuessOk() throws Exception {
        ...
        int i = 0;
        for (String name : fakeNames) {
            assertEquals("Console output isn't as expected", String.format("[For player %d] What's your name?", i),
                    runOutput.readLine().trim());
            assertEquals("Console output isn't as expected", String.format("[For everyone] %s joined", name),
                    runOutput.readLine().trim());
            i++;
        }
        for (i = 0; i < numMoves; ++i) {
            assertEquals("Console output isn't as expected",
                    String.format("[For %s] enter your guess->", fakeNames[i % fakeNames.length]),
                    runOutput.readLine().trim());
            assertEquals("Console output isn't as expected",
                    String.format("[For %s] %d", fakeNames[i % fakeNames.length], i * 97 % 100),
                    runOutput.readLine().trim());
        }
